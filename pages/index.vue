<template>
  <div>
    ...
    <div class="container search">
      <input @keyup.enter="$fetch" type="text" name="searchInput" id="searchInput" v-model.lazy="searchInput" placeholder="Search...">
      ...
      <div v-if="searchInput !== ''" id="movie-grid" class="movies-grid">
        <div class="movie" v-for="(movie, index) in searchedMovies" :key="index">
          ...
          <div class="info">
            ...
            <p class="release">Released: {{ new Date(movie.release_date).toLocaleDateString('pt-br') }}</p>
            <nuxt-link class="button button-light" :to="{name: 'movies-id', params: { id: movie.id }}">Get More Info</nuxt-link>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    async getMovies() {
      const data = this.$axios.get('https://api.themoviedb.org/3/movie/now_playing?api_key=da2bc268e07d9b4f763d9274e370a55a&language=pt-BR')
